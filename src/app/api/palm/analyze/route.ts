...
export async function POST(request: Request) {
  try {
    ...
    if (sessionVerifyError) {
      console.error("[POST /api/palm/analyze] Session verification error:", sessionVerifyError);
      return errorResponse("BAD_REQUEST", `无法验证 session: ${sessionVerifyError.message}`, 500);
    }
    
    if (!sessionVerify) {
      console.error("[POST /api/palm/analyze] Session does not exist after verifyOrCreateSession:", sessionId);
